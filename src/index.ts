...
type Scope = NodePath<t.Node>["scope"]

const viteSloppyMode = (params?: sloppyModeParams) => {
  ...
  const findInScope = (scope: Scope, name: string) => {
    let hasFound = false;
    const traverse = (innerScope: Scope) => {
